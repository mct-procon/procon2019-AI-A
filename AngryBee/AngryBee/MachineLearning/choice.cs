...
namespace AngryBee.MachineLearning
{
    public class choice
    {
        ...
        public const int ParaNum = 2;
        public double[,] DNAs = new double[SampleNum, ParaNum];
        private double[,] NextDNAs = new double[SampleNum, ParaNum];
        ...
        public void WriteFile()
        {
            ...
            System.IO.StreamWriter beforefile = new System.IO.StreamWriter(@"C:\Users\samin\OneDrive\ドキュメント\procon2019\machine_learning\before.txt", false);
            file.Close();
            file = new System.IO.StreamWriter(@"C:\Users\samin\OneDrive\ドキュメント\procon2019\machine_learning\test.txt", true);
            beforefile.Close();
            beforefile = new System.IO.StreamWriter(@"C:\Users\samin\OneDrive\ドキュメント\procon2019\machine_learning\before.txt", true);
            ...
            for (int i = 0; i < SampleNum; i++)
            {
                beforefile.Write("{0}:", WinPoint[i]);
                for (int j = 0; j < ParaNum; j++)
                {
                    beforefile.Write("{0} ", DNAs[i, j]);
                }
                beforefile.Write("\n");
            }
