...
{
    public class AgentDistanceAI : MCTProcon30Protocol.AIFramework.AIBase
    {
        PointEvaluator.Base PointEvaluator_AgentDistance = new PointEvaluator.AgentDistance();
        ...
        public AgentDistanceAI(int startDepth = 0)
        {
            for (int i = 0; i < 50; ++i)
            {
                dp1[i] = new DP();
        ...
        protected override void Solve()
        {
            ...
            PointEvaluator.Base evaluator = (TurnCount / 3 * 2) < CurrentTurn ? PointEvaluator_Normal : PointEvaluator_AgentDistance;
        ...
        private int NegaMax(int deepness, SearchState state, int alpha, int count, PointEvaluator.Base evaluator, Decision ngMove, Unsafe8Array<Way> nextways, int nowAgent, int watch_deepness)
        {
            ...
            foreach (var way in ways.Data[nowAgent])
            {
                ...
                if (count == 0)
                {
                    ...
                    {
                        if (ngMove is null && way.Locate == dp1[0].Ways[j].Locate) break;
                        else if (!(ngMove is null) && way.Locate == dp2[0].Ways[j].Locate) break;
                    }
